...
interface RecipeFormProps {
  isOpen: boolean;
  editingRecipe: { id: string; name: string; description?: string; notes?: string; recipeProducts?: Array<{ productId: string; quantity: number; matchingRuleId?: string | null; product?: { id: string; name: string; type: string; cost: number } }> } | null;
  token: string | null;
  products: Product[];
...
export default function RecipeForm({ isOpen, editingRecipe, token, products, matchingRules, showToast, onClose, onSuccess }: RecipeFormProps) {
      ...
      <div className="modal-content max-w-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <input className="input" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required placeholder="VD: Móc khóa Custom" />
          </div>
          <div>
            <label className="label">Mô tả</label>
            <textarea className="input" rows={2} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Mô tả sản phẩm bán ra" />
          </div>
          ...
          <div className="bg-gradient-to-br from-blue-50 to-blue-50/30 rounded-xl p-5 border border-blue-100/80 transition-all duration-200">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] font-semibold text-blue-600 bg-blue-100/80 px-2.5 py-1 rounded-full">Bắt buộc</span>
            </div>
            {baseProducts.length === 0 ? (
              <div className="flex items-center gap-2 p-3 bg-amber-50 rounded-lg border border-amber-200">
                <FlaticonIcon name="triangle-warning" size="sm" className="text-amber-500" />
                <p className="text-xs text-amber-700">Chưa có sản phẩm BASE. Vui lòng tạo nguyên vật liệu BASE trước.</p>
              </div>
            ) : (
              <div className="relative">
                <select className="input w-full pr-24 appearance-none" value={form.baseProductId} onChange={(e) => setForm({ ...form, baseProductId: e.target.value })}>
                  {baseProducts.map((p) => (
          ...
          <div className="bg-gradient-to-br from-pink-50 to-pink-50/30 rounded-xl p-5 border border-pink-100/80 transition-all duration-200">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-[10px] font-semibold text-pink-600 bg-pink-100/80 px-2.5 py-1 rounded-full">Không bắt buộc</span>
            </div>

            {form.charmProducts.length > 0 && (
              <div className="hidden sm:grid grid-cols-[7fr_3fr_auto] gap-2 px-1 mb-1.5">
                <span className="text-[10px] font-medium text-gray-400 uppercase tracking-wider">Sản phẩm</span>
                <span className="text-[10px] font-medium text-gray-400 uppercase tracking-wider">Quy tắc ghép</span>
              </div>
            )}
            ...
            {form.charmProducts.map((cp, idx) => {
              ...
              return (
                <div key={idx} className="bg-white rounded-lg border border-pink-200/60 p-2.5 shadow-sm mb-2 last:mb-3 transition-all duration-200 hover:border-pink-300/80 hover:shadow-md">
                  {/* Mobile layout */}
                  <div className="flex items-center gap-2 mb-2 sm:hidden">
                    <select className="input text-sm flex-1 appearance-none" value={cp.productId} onChange={(e) => {
                    ...
                    <button type="button" onClick={() => setForm({ ...form, charmProducts: form.charmProducts.filter((_, i) => i !== idx) })} className="w-7 h-7 bg-red-50 text-red-400 rounded-lg flex items-center justify-center hover:bg-red-100 hover:text-red-600 transition-all duration-200 flex-shrink-0" aria-label="Xóa CHARM"><FlaticonIcon name="x" size="xs" /></button>
                  </div>
                  <div className="sm:hidden space-y-2">
                    {cp.productId && (availableRules.length > 0 ? (
                      <select className="input text-xs w-full appearance-none" value={cp.matchingRuleId} onChange={(e) => {
                        const cps = [...form.charmProducts];
                      ...
                      <div className="text-[11px] text-amber-600 bg-amber-50 px-2.5 py-1.5 rounded-md text-center flex items-center justify-center gap-1">
                        <FlaticonIcon name="triangle-warning" size="xs" className="text-amber-500" /> Hết quy tắc
                      </div>
                    ))}
                    <p className="text-xs text-gray-500">
                      Giá vốn: <span className="font-semibold text-pink-700">{selectedProduct ? formatCurrency(Number(selectedProduct.cost)) : '0₫'}</span>
                    </p>
                  </div>
                  ...
                  <div className="hidden sm:grid sm:grid-cols-[7fr_3fr_auto] gap-2 items-center">
                    <div className="relative">
                      <select className="input text-xs w-full pr-24 appearance-none" value={cp.productId} onChange={(e) => {
                        const cps = [...form.charmProducts];
                        cps[idx] = { productId: e.target.value, matchingRuleId: '' };
                        setForm({ ...form, charmProducts: cps });
                      }}>
                    ...
                    <button type="button" onClick={() => setForm({ ...form, charmProducts: form.charmProducts.filter((_, i) => i !== idx) })} className="w-7 h-7 bg-red-50 text-red-400 rounded-lg flex items-center justify-center hover:bg-red-100 hover:text-red-600 transition-all duration-200 flex-shrink-0 justify-self-center" aria-label="Xóa CHARM"><FlaticonIcon name="x" size="xs" /></button>
                  </div>
                </div>
              );
            })}
            ...
            {(() => {
              const allUsed = charmProductsMaster.length > 0 && charmProductsMaster.every((p) => form.charmProducts.some((cp) => cp.productId === p.id));
              return !allUsed ? (
                <button type="button" onClick={() => setForm({ ...form, charmProducts: [...form.charmProducts, { productId: '', matchingRuleId: '' }] })} className="w-full py-2.5 border-2 border-dashed border-pink-200/70 rounded-lg text-sm font-medium text-pink-500 hover:text-pink-700 hover:border-pink-300 hover:bg-pink-50/50 transition-all duration-200 flex items-center justify-center gap-2 group/add">
                  <span className="w-5 h-5 rounded-full bg-pink-100 flex items-center justify-center text-xs group-hover/add:bg-pink-200 transition-colors">+</span>
                  Thêm CHARM
                </button>
              ) : null;
            })()}
          ...
          <div className="flex gap-3 justify-end pt-2 border-t border-gray-100">
            ...
            <button type="submit" className="btn-primary">{editingRecipe ? 'Cập nhật' : 'Tạo sản phẩm'}</button>
          </div>
        </form>
      </div>
    </div>
