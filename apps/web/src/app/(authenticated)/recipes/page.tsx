...
import { ProductTypeBadge, type Recipe, type RecipeProduct, type Product, type MatchingRule } from './recipeConstants';
...
export default function RecipesPage() {
  ...
  const confirmDelete = (recipe: Recipe) => {
    ...
    overlay.className = 'fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-[fadeIn_0.15s_ease-out]';
    ...
    `;
    dialog.querySelector('#cancel-btn')!.addEventListener('click', () => document.body.removeChild(overlay));
    ...
    <div className="page-enter">
        ...
        <div className="relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">Công thức định nghĩa sản phẩm cuối cùng</p>
            </div>
          </div>
          <button
      ...
      <div className="action-bar">
        <div className="relative flex-1 max-w-xs">
          ...
          <input className="input pl-9" placeholder="Tìm kiếm sản phẩm..." value={search} onChange={(e) => { setSearch(e.target.value); setPage(1); }} />
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {filteredRecipes.map((recipe) => {
            const totalCost = (recipe.recipeProducts || []).reduce((s, rp) => s + Number(rp.product?.cost || 0) * rp.quantity, 0);
            const baseRp = (recipe.recipeProducts || []).find((rp) => rp.product?.type === 'BASE');
            const charmRps = (recipe.recipeProducts || []).filter((rp) => rp.product?.type === 'CHARM');
            ...
            return (
              <div key={recipe.id} className="card p-0 overflow-hidden group">
                <div className="p-5 flex items-center justify-between cursor-pointer hover:bg-gray-50/50 transition-colors" onClick={() => setExpandedRecipe(expandedRecipe === recipe.id ? null : recipe.id)}>
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        {recipe.description && <p className="text-sm text-gray-500 truncate">{recipe.description}</p>}
                        {baseRp && <span className="badge-blue text-[10px]">1 nền</span>}
                        {charmRps.length > 0 && <span className="text-xs text-gray-400">+{charmRps.length} CHARM</span>}
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <div className="text-right hidden sm:block">
                      ...
                      <p className="font-bold text-gray-900 tabular-nums">{formatCurrency(totalCost)}</p>
                    </div>
                    <div className="flex gap-1">
                      <button onClick={(e) => { e.stopPropagation(); confirmDelete(recipe); }} className="btn-ghost btn-xs opacity-0 group-hover:opacity-100 transition-opacity hover:text-red-600" title="Xóa"><FlaticonIcon name="trash" size="sm" /></button>
                      <button onClick={(e) => { e.stopPropagation(); handleDuplicate(recipe); }} className="btn-ghost btn-xs opacity-0 group-hover:opacity-100 transition-opacity" title="Nhân bản"><FlaticonIcon name="clipboard" size="sm" /></button>
                      <button onClick={(e) => { e.stopPropagation(); setEditingRecipe(recipe); setShowForm(true); }} className="btn-ghost btn-xs" aria-label="Chỉnh sửa"><FlaticonIcon name="pencil" size="sm" /></button>
                      <span className={'text-gray-400 transition-transform duration-200 ' + (expandedRecipe === recipe.id ? 'rotate-90' : '')}><FlaticonIcon name="angle-right" size="sm" /></span>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="mx-5 mb-5 p-5 bg-gray-50 rounded-xl animate-[slideDown_0.2s_ease-out]">
                    {baseRp && (
                      <div className="mb-5">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Nền tảng</h4>
                        </div>
                        <div className="bg-white rounded-xl p-4 border border-blue-100 shadow-sm hover:shadow-md transition-all duration-200">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3.5">
                              ...
                              <div>
                                <div className="flex items-center gap-2.5">
                                  <p className="text-sm font-semibold text-gray-900">{baseRp.product.name}</p>
                                  <ProductTypeBadge type="BASE" />
                                </div>
                                <p className="text-[11px] text-gray-400 mt-0.5">Giá vốn: {formatCurrency(Number(baseRp.product.cost))}</p>
                              </div>
                            </div>
                            <div className="text-right">
                              <p className="text-xs text-gray-400 mb-0.5">Chi phí</p>
                              <p className="text-base font-bold text-gray-900 tabular-nums">{formatCurrency(Number(baseRp.product.cost))}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mb-5">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">
                            ...
                            <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Phụ kiện bổ sung</h4>
                          </div>
                          <span className="text-[10px] text-gray-400 bg-white px-2 py-0.5 rounded-full">{charmRps.length} cái</span>
                        </div>
                        <div className="space-y-2">
                          {charmRps.map((rp) => {
                            const ruleCode = getRuleCode(rp);
                            return (
                              <div key={rp.id} className="bg-white rounded-xl p-4 border border-pink-100 shadow-sm hover:shadow-md hover:border-pink-200 transition-all duration-200">
                                <div className="flex items-center justify-between">
                                  <div className="flex items-center gap-3.5">
                                    ...
                                    <div>
                                      <div className="flex items-center gap-2.5">
                                        <p className="text-sm font-semibold text-gray-900">{rp.product.name}</p>
                                        <ProductTypeBadge type="CHARM" />
                                      </div>
                                      <p className="text-[11px] text-gray-400 mt-0.5">
                                        Giá vốn: <span className="tabular-nums">{formatCurrency(Number(rp.product.cost))}</span>
                                        {ruleCode && (
                                          <span className="ml-2 inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-purple-50 text-purple-600 font-medium">
                                            <FlaticonIcon name="text" size="xs" className="text-purple-500" /> {ruleCode}
                                          </span>
                                        )}
                                      </p>
                                    </div>
                                  ...
                                  <div className="text-right">
                                    <p className="text-xs text-gray-400 mb-0.5">Chi phí</p>
                                    <p className="text-base font-bold text-gray-900 tabular-nums">{formatCurrency(Number(rp.product.cost))}</p>
                                  </div>
                                </div>
                              </div>
                            );
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <h4 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Tổng chi phí</h4>
                      </div>
          ...
          {filteredRecipes.length === 0 && (
            <EmptyState emoji="📋" title={search ? 'Không tìm thấy phù hợp' : 'Chưa có sản phẩm bán nào'} message={search ? 'Thử từ khóa khác.' : 'Tạo sản phẩm bán đầu tiên từ công thức.'} />
          )}
